...
const AudioCall= ({navigation}) => {
    ...
    return (
        <View className="h-full w-screen bg-zinc-950 flex">
            <ExpoStatusBar />
            <CameraView className="" ref={cameraRef} facing={facing}></CameraView>
            <TouchableOpacity
                onPress={() => {
            ...
            <View className="w-screen bg-zinc-800 items-center h-20 rounded-t-2xl bottom-0 absolute flex flex-row justify-evenly">
                <TouchableOpacity onPress={toogleCameraFacing} className="flex justify-center items-center h-14 w-14 rounded-full">
                    <Icon name="video" color="gainsboro" size={30} />
                </TouchableOpacity>
            ...
            <View className="absolute top-[40%] w-screen items-center flex">
                <Image source={require('./Images/image3.jpg')} resizeMode="cover" className="h-44 w-44 rounded-full" />
            </View>
        </View>
    );
};
